...
-- Hàm chuyển màu mượt
function smoothRainbowTransition(part, speed)
    local rainbowColors = {
    ...
    while true do
        for i = 1, #rainbowColors do
            ...
            local endColor = rainbowColors[i % #rainbowColors + 1]
            
            for t = 0, 1, speed do
                part.Color = startColor:Lerp(endColor, t)
                wait(0.02)  -- Thời gian chờ giữa các bước chuyển
            end
        end
    end
end
...
function onTouch(hit)
    local character = hit.Parent -- Lấy Character từ đối tượng va chạm

    -- Kiểm tra xem Character có Humanoid không
    ...
    if humanoid then
        ...
        -- Bạn có thể thêm hiệu ứng, âm thanh, hoặc hiệu ứng đặc biệt khi bị mất máu ở đây
    end
end

-- Tạo một Part mới cho trái cây
local Fruit = Instance.new("Part")
Fruit.Name = "Fake Test Fruit"
Fruit.Parent = game:GetService("Workspace")

-- Thiết lập kích thước, màu sắc và thuộc tính cho trái cây
Fruit.Size = Vector3.new(5, 5, 5)
Fruit.Color = Color3.fromRGB(255, 165, 0)
Fruit.Anchored = false  -- Đặt thành false để cho phép va chạm
Fruit.CanCollide = true  -- Cho phép va chạm với các đối tượng khác

-- Đặt vị trí cho trái cây
Fruit.Position = Vector3.new(-290.7376708984375, 6.729952812194824, 5343.5537109375)

-- Gọi hàm smooth rainbow transition (đảm bảo hàm này đã được định nghĩa)
smoothRainbowTransition(Fruit)

-- Định nghĩa hàm onTouch
local function onTouch(hit)
    local player = game.Players:GetPlayerFromCharacter(hit.Parent)
    if player then
        print(player.Name .. " đã chạm vào trái cây!")
        -- Thực hiện hành động khi chạm vào
    end
end

-- Kết nối sự kiện Touched cho trái cây
Fruit.Touched:Connect(onTouch)

-- Tạo một Handle cho việc tween teleport
local Handle = Instance.new("Part")
Handle.Name = "Handle"
Handle.Size = Vector3.new(1, 1, 1)  -- Kích thước tùy ý
Handle.Color = Color3.new(0, 1, 0)  -- Màu sắc tùy ý
Handle.Anchored = true  -- Neo Handle lại để không di chuyển
Handle.CanCollide = false  -- Không cho phép va chạm
Handle.Parent = Fruit  -- Thêm Handle vào Fruit

-- Đặt vị trí cho Handle
Handle.Position = Fruit.Position  -- Đặt vị trí của Handle giống như Fruit
