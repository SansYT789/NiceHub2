...
-- Disable Camera Shaking and set initial combat properties
spawn(function()
    game:GetService("RunService").RenderStepped:Connect(function()
        if typeof(CbFw2) == "table" then
            pcall(function()
                CameraShaker:Stop()
                CbFw2.activeController.timeToNextAttack = 0
                CbFw2.activeController.hitboxMagnitude = 180
                CbFw2.activeController.active = false
                CbFw2.activeController.timeToNextBlock = 0
                CbFw2.activeController.focusStart = tick()  -- use tick for better stability
...
-- Reset character state properties
spawn(function()
    game:GetService("RunService").RenderStepped:Connect(function()
        if character then
            local stun = character:FindFirstChild("Stun")
            local busy = character:FindFirstChild("Busy")
            if stun and busy then
                pcall(function()
                    stun.Value = 0
                    busy.Value = false
                end)
...
-- Function to get the current blade equipped by the player
...
-- Function to perform an attack without cooldown
function AttackNoCD()
    CameraShaker:Stop()
    ...
    for _, v in pairs(bladehit) do
        if v.Parent:FindFirstChild("HumanoidRootPart") and not hitSet[v.Parent] then
            table.insert(targets, v.Parent.HumanoidRootPart)
            hitSet[v.Parent] = true
        end
    end
...
-- Loop to continuously attack when FastAttackNice is not active
spawn(function()
    while task.wait() do
...
-- Farming enemy NPCs within range
task.spawn(function()
    while task.wait() do
        pcall(function()
            for _, v in pairs(game.Workspace.Enemies:GetChildren()) do
                ...
                if enemyHumanoid and enemyRoot and enemyHumanoid.Health > 0 then
                    if v.Name == MonFarm and (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - enemyRoot.Position).Magnitude <= 100 then
                        repeat
                            task.wait()
                            AutoHaki()
                            Click()
                        ...
                        until (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - enemyRoot.Position).Magnitude > 100
