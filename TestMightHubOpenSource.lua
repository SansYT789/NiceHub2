...
local PosY = 25 -- Giá trị của PosY có thể được thay đổi tại đây
ChangeSpeed = 350
_G.AutoKaitun = true
...
local NoAttackAnimation, Fast_Attack, DamageAura = true, true, true
...
until game:GetService("Players").LocalPlayer.Backpack:FindFirstChild("Yama") or game:GetService("Players").LocalPlayer.Character:FindFirstChild("Yama") or GetWeaponInventory("Yama")
end
...
if string.find(spawn.Name, Ms) then
...
spawn(function()
    while task.wait(.125) do
        pcall(function()
            for _, enemy in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                if not string.find(enemy.Name,"Boss") and enemy.Name == MonName and 
                   (enemy.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 450 then
