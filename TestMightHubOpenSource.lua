...
spawn(function()
    while wait() do
        pcall(function()
            ...
            -- Đợi 0.1 giây trước khi lặp lại
            wait(.15)
        end)
    end
end)
...
local NoAttackAnimation, Fast_Attack = true, true
...
MonPos = elite.HumanoidRootPart.CFrame
MonName = elite.Name
HackerNoclipFly = true
NeedAttacking = true
...
MonPos = enemy.HumanoidRootPart.CFrame
MonName = enemy.Name
HackerNoclipFly = true
NeedAttacking = true
sethiddenproperty(game.Players.LocalPlayer, "SimulationRadius", math.huge)
            ...
            for _, enemy in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
                if not string.find(enemy.Name,"Boss") and enemy.Name == MonName and 
                   (enemy.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 450 then
                    ...
                    enemy.Humanoid:ChangeState(16) -- Seated
