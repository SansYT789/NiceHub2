...
function Tween(P1)
local TweenSpeed = ChangeSpeed or 325

Distance = Bucac(HMRP, P1)
Portal = getPortal(P1)
DefaultY = P1.Y
BypassCheck = CheckBypassTrue(P1)
targetCFrameWithDefaultY = CFrame.new(P1.X, DefaultY, P1.Z)
    ...
    while wait() do
        pcall(function()
            local RandomCFrameChoices = {
                CFrame.new(0, posY, 0),
                CFrame.new(15, posY, 20),
                CFrame.new(-20, posY, -15),
                CFrame.new(0, posY, 20),
                CFrame.new(0, posY, -20)
            }
                    ...
                    if _G.AutoMasteryOption and KillPercent and targetMob.Humanoid.MaxHealth < 200000 then
                        ...
                        if lowHealth then
                            repeat
                                ...
                                if string.find(skillName or "", "Dough") then
                                    game:GetService('VirtualUser'):CaptureController()
                                    game:GetService('VirtualUser'):Button1Down(Vector2.new(1280, 672))
                                end
...
-- Initialize SkillsSelected from saved flags
if OrionLib.Flags["SkillsSelected"].Value then
    SkillsSelected = OrionLib.Flags["SkillsSelected"].Value
end

-- Populate RealSkillSelected based on current SkillsSelected
...
-- Add Dropdown for Skill Selection
M:AddDropdown({
    Name = "Select Skills",
    Default = "",
    ...
    Callback = function(selectedSkill)
        -- Toggle selection status of the selected skill
        SkillsSelected[selectedSkill] = not SkillsSelected[selectedSkill]
        
        -- Update RealSkillSelected with only active skills
        RealSkillSelected = {}
        for skill, isSelected in pairs(SkillsSelected) do
            if isSelected then
                table.insert(RealSkillSelected, skill)
...
if v.Name == MonFarm and (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - v.HumanoidRootPart.Position).Magnitude <= posY + 55 then
repeat task.wait()
AutoHaki()
...
for i,v in pairs(game.Workspace.Enemies:GetChildren()) do
if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid").Health > 0 then
if (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - v.HumanoidRootPart.Position).Magnitude <= posY + 75 then
repeat task.wait()
AutoHaki()
Click()
until (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - v.HumanoidRootPart.Position).Magnitude > posY + 75 or not v.Parent or v.Humanoid.Health <= 0
end
end
end
end
...
-- Chức năng để kéo quái về vị trí Farm
task.spawn(function()
    while task.wait() do
        if _G.BringMob then
            pcall(function()
...
-- Hàm kiểm tra đối tượng có nằm trong mạng của người chơi không
