...
function BypassTp(p)
    ...
    repeat
        ...
        -- Thay vì phá hủy Head, chỉ cần làm cho nó trong suốt và vô hình
        HM.Health = 0
    ...
    -- Khôi phục lại Head nếu cần thiết
...
function Tween(P1)
    if not HMRP or HM.Health <= 0 then
        return
    end
...
if NoClip then
if not game:GetService("Players").LocalPlayer.Character.HumanoidRootPart:FindFirstChild("MightHubTween") then
...
elseif not NoClip and game:GetService("Players").LocalPlayer.Character.HumanoidRootPart:FindFirstChild("MightHubTween") then
game:GetService("Players").LocalPlayer.Character.HumanoidRootPart:FindFirstChild("MightHubTween"):Destroy()
end
end)
end)
        ...
        if type(Mob) == "table" then
            ...
            elseif c2 then
                ...
                if string.find(game:GetService("Workspace")["_WorldOrigin"].EnemySpawns:GetChildren()[1].Name, "Lv.") then
                    ...
                    for _, mobName in pairs(Mob) do
                        table.insert(mobSpawnList, tostring(mobName:gsub(" %pLv. %d+%p", "")))
                    end
                end
                
                for _, mobName in pairs(mobSpawnList) do
                    local mobSpawn = game:GetService("Workspace")["_WorldOrigin"].EnemySpawns:FindFirstChild(mobName)
                    
                    if mobSpawn and not CheckMob(Mob) then
                        local target = mobSpawn:IsA("Part") and mobSpawn or mobSpawn:FindFirstChild("HumanoidRootPart")
                        if target then Tween(target.CFrame * MethodFarm) end
                    end
                end
            end
        end
...
game.Players.LocalPlayer.Character:FindFirstChild("Stun").Changed:Connect(function()
