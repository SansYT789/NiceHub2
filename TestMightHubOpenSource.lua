...
until game.Players 
    ...
    and game.Players.LocalPlayer.Character:FindFirstChild("Energy")
...
_G.BringMob = _G.BringMob or true
function InMyNetWork(object)
    if isnetworkowner then
        return isnetworkowner(object)
    else
        -- Nếu không có isnetworkowner, sử dụng khoảng cách để xác định
        return (object.Position - HMRP.Position).Magnitude <= 450
    end
end
task.spawn(function()
    while task.wait() do
...
function BringMob(MonFarm, FarmPos)
    -- Iterate through all enemies in the workspace
    for _, v in pairs(game.Workspace.Enemies:GetChildren()) do
        -- Check if the enemy is valid and alive
        if v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
            -- Check if the enemy matches the desired mob and is within range
            if v.Name == MonFarm and (v.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 400 then
                -- Check if the enemy is in the player's network
                if InMyNetWork(v.HumanoidRootPart) then
                    -- Teleport the mob to the farm position
                    v.HumanoidRootPart.CFrame = FarmPos
                    -- Disable collision and set size/transparency
                    v.HumanoidRootPart.CanCollide = false
                    v.HumanoidRootPart.Size = Vector3.new(1, 1, 1)
                    v.HumanoidRootPart.Transparency = 1
                    
                    local humanoid = v:FindFirstChild("Humanoid")
                    -- Change humanoid states to disable movement
                    humanoid:ChangeState(11)  -- Some state (Idle)
                    humanoid:ChangeState(14)  -- Some state (Falling)
                    humanoid:ChangeState(16)  -- Some state (Seated)
                    humanoid.JumpPower = 0
                    humanoid.WalkSpeed = 0
...
if v.Name == bk and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
return v
end
end
if bl then
...
if not target then
if game:GetService("Players").LocalPlayer.Character.HumanoidRootPart:FindFirstChild("MightHubTween") then
game:GetService("Players").LocalPlayer.Character.HumanoidRootPart:FindFirstChild("MightHubTween"):Destroy()
end
_G.StopTween = true
...
function CheckSkillMastery()
    if not _G.SelectMethodFarmMastery then
        return
    end

    local MasteryWeapon

    if _G.SelectMethodFarmMastery == "Blox Fruit" then
        MasteryWeapon = game:GetService("Players").LocalPlayer.Data.DevilFruit.Value
    elseif _G.SelectMethodFarmMastery == "Gun" then
        MasteryWeapon = ""
        local BackpackAndCharacter = {game.Players.LocalPlayer.Backpack, game.Players.LocalPlayer.Character}

        for _, container in pairs(BackpackAndCharacter) do
            for _, item in pairs(container:GetChildren()) do
                if item:IsA("Tool") and item.ToolTip == "Gun" then
                    MasteryWeapon = item.Name
                    break
                end
            end
            if MasteryWeapon ~= "" then break end
        end
    ...
    if MasteryWeapon and MasteryWeapon ~= "" then
        ...
        local backpackOrCharacter = player.Backpack:FindFirstChild(MasteryWeapon) or player.Character:FindFirstChild(MasteryWeapon)
        local skillGui = player.PlayerGui.Main.Skills:FindFirstChild(MasteryWeapon)

        if backpackOrCharacter and skillGui then
            for _, skillFrame in pairs(skillGui:GetChildren()) do
                if skillFrame:IsA("Frame") and skillFrame.Name ~= "Template" and table.find(RealSkillSelected, skillFrame.Name) then
                    local titleColor = skillFrame.Title.TextColor3
                    local cooldownSize = skillFrame.Cooldown.Size

                    if titleColor == Color3.new(1, 1, 1) and (cooldownSize == UDim2.new(0, 0, 1, -1) or cooldownSize == UDim2.new(1, 0, 1, -1)) then
                        return MasteryWeapon, skillFrame.Name
                    end
                end
            end
        else
            EquipTool(MasteryWeapon)
        end
    end
end
...
loadstring(
    ...
    gt.__namecall = newcclosure(function(...)
        ...
        if getnamecallmethod() == "InvokeServer" then 
            if tostring(args[2]) == "TAP" then
                if AimbotCheck and AimbotMobPosition then
                    args[3] = AimbotMobPosition
                end
            end
        end
    ...
    setreadonly(gt, true) -- Đặt metatable về trạng thái chỉ đọc để tránh lỗi
]]
)()
...
function KillMob(Mob, c2)
    pcall(function()
        -- Kiểm tra nếu Mob là một bảng
        if type(Mob) ~= "string" then return end

        local targetMob = CheckMob(Mob)

        -- Nếu mob hợp lệ và có sức khỏe, bắt đầu chiến đấu
        if targetMob and targetMob:FindFirstChild("Humanoid") and targetMob:FindFirstChild("HumanoidRootPart") and targetMob.Humanoid.Health > 0 then
            repeat
                task.wait()
                Click()
                AutoHaki()
                EquipTool(SelectWeapon)
                Tween(targetMob.HumanoidRootPart.CFrame * MethodFarm)
                FarmPos, MonFarm = targetMob.HumanoidRootPart.CFrame, targetMob.Name
                BringMob(targetMob.Name, targetMob.HumanoidRootPart.CFrame)

                -- Kiểm tra nếu AutoMastery được bật và ngưỡng sức khỏe mob đạt yêu cầu
                if _G.AutoMasteryOption and KillPercent and targetMob.Humanoid.MaxHealth < 200000 then
                    local lowHealth = targetMob.Humanoid.Health <= targetMob.Humanoid.MaxHealth * KillPercent / 100
                    _G.FastAttackNice = not lowHealth
                    NormalAttack()
                    AutoHaki()

                    -- Nếu mob có sức khỏe thấp, chuyển sang chế độ tấn công mastery
                    if lowHealth then
                        repeat
                            task.wait()
                            local skillName, skillKey = CheckSkillMastery()
                            Tween(targetMob.HumanoidRootPart.CFrame * MethodFarm)
                            AimbotMobPosition, AimbotCheck = targetMob.HumanoidRootPart.Position, true

                            spawn(function()
                                if _G.SelectMethodFarmMastery == "Gun" and skillName then
                                    EquipTool(skillName)
                                    ClickCamera()
                                    game:GetService("Players").LocalPlayer.Character[skillName].RemoteEvent:FireServer(targetMob.HumanoidRootPart.Position)
                                    game:GetService("Players").LocalPlayer.Character[skillName].RemoteFunctionShoot:InvokeServer(targetMob.HumanoidRootPart.Position, targetMob.HumanoidRootPart)
                                end
                            end)

                            if string.find(skillName or "", "Dough") or string.find(skillName or "", "Leopard") or string.find(skillName or "", "Spirit") then
                                ClickCamera()
                            elseif string.find(skillName or "", "Light") or string.find(skillName or "", "Ice") then
                                Click()
                            end

                            if skillName and skillKey then
                                EquipTool(skillName)
                                Char:FindFirstChild(skillName).MousePos.Value = AimbotMobPosition
                                KeyPress(skillKey)
                                task.wait(0.2)
                            end
                        until not lowHealth or not targetMob or targetMob.Humanoid.Health <= 0 or CheckIsRaiding()

                        AimbotCheck, AimbotMobPosition = false, nil
                    end
                end
            until not targetMob or targetMob.Humanoid.Health <= 0 or CheckIsRaiding()

            -- Đặt lại các biến trạng thái
            _G.FastAttackNice, AimbotCheck, AimbotMobPosition = false, false, nil
        end
        
        -- Nếu không tìm thấy mob hợp lệ và có c2, teleport đến spawn
        if not targetMob and c2 then
            for _, spawn in pairs(game:GetService("Workspace")["_WorldOrigin"].EnemySpawns:GetChildren()) do
                if string.find(spawn.Name, Mob.Name) then
...
M:AddDropdown({
	...
	Default = "Level",
	Options = {"Level", "Nearest", "Cake Prince", "Bone"},
	Flag = "SelectMethodFarm",
    Save = true,
	Callback = function(Value)
	ChooseMethodFarm = Value
	end    
})
...
Tween(targetMob.HumanoidRootPart.CFrame * MethodFarm)
FarmPos, MonFarm = targetMob.HumanoidRootPart.CFrame, targetMob.Name
BringMob(targetMob.Name, CFrameMon)
...
until not _G.AutoFarmAcceptQuest or not _G.AutoLevel or CheckIsRaiding() or not v.Parent or v.Humanoid.Health <= 0 or not game:GetService("Workspace").Enemies:FindFirstChild(v.Name) or game.Players.LocalPlayer.PlayerGui.Main.Quest.Visible == false
end
end
end
for _, spawn in pairs(game:GetService("Workspace")["_WorldOrigin"].EnemySpawns:GetChildren()) do
...
repeat game:GetService("RunService").Heartbeat:wait()
KillMob(v)
until _G.AutoFarmAcceptQuest or not _G.AutoLevel or not v.Parent or v.Humanoid.Health <= 0 or not game:GetService("Workspace").Enemies:FindFirstChild(v.Name)
end
end
end
Tween(CFrameMon)
...
M:AddToggle({
	Name = "Auto Farm Mastery Option",
	Default = false,
	Flag = "AutoFarmMastery",
    Save = true,
	Callback = function(Value)
	_G.AutoMasteryOption = Value
	end    
})
...
SkillsSelected = {}
RealSkillSelected = {}
if SkillsSelected then
SkillsSelected = SkillsSelected
end

for skill, isSelected in pairs(SkillsSelected) do
if isSelected then
table.insert(RealSkillSelected, skill)
end
end
...
M:AddDropdown({
    ...
    Flag = "SkillsSelected",
    Save = true,
    Callback = function(selectedSkill)
SkillsSelected[selectedSkill] = not SkillsSelected[selectedSkill]
RealSkillSelected = {}
for skill, isSelected in pairs(SkillsSelected) do
if isSelected then
table.insert(RealSkillSelected, skill)
...
OrionLib:MakeNotification({
	Name = "Notifying",
	Content = "Skill " .. tostring(selectedSkill) .. ": " ..tostring(SkillsSelected[selectedSkill]),
	Image = "rbxassetid://4483345998",
	Time = 3.5
})
    end    
...
Tween(game:GetService("ReplicatedStorage"):FindFirstChild("Darkbeard").HumanoidRootPart.CFrame * MethodFarm)
end
end
end)
end
...
until not getNextIsland() or not CheckIsRaiding() or not _G.OneClickRaid
end
game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("Awakener","Awaken")
...
spawn(function()
while task.wait() do
if _G.AutoNextIsland and CheckIsRaiding() then
pcall(function()
if getNextIsland() then
repeat wait()
Tween(getNextIsland().CFrame * RaidPos)
until not getNextIsland() or not CheckIsRaiding() or not _G.AutoNextIsland
end
end)
end
end
...
local listfastattack = {"0.5","0.4","0.3","0.2","0.15","0.1","0"}
...
if (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - v.HumanoidRootPart.Position).Magnitude <= 75 then
repeat task.wait()
AutoHaki()
Click()
until (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - v.HumanoidRootPart.Position).Magnitude > 75 or not v.Parent or v.Humanoid.Health <= 0
end
end
end
end)
