...
spawn(function()
while task.wait() do
if MonPos and MonName then
pcall(function()
for _, enemy in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
if enemy:FindFirstChild("Humanoid") and enemy:FindFirstChild("HumanoidRootPart") and enemy.Humanoid.Health > 0 then
if enemy.Name == MonName and (enemy.HumanoidRootPart.Position - MonPos.Position).Magnitude <= 450 then
repeat task.wait()
...
until not enemy.Parent or enemy.Humanoid.Health <= 0 or not MonPos or not MonName
end
end
end
end)
