    ...
    for _, scriptName in pairs(scriptNames) do
        for _, script in pairs(parent:GetDescendants()) do
            if script:IsA("LocalScript") and script.Name == scriptName then
                script:Destroy()
                print("Destroyed script: " .. scriptName)  -- User feedback
            end
        end
    end
end
...
function Tween(P1)
    if game.Players.LocalPlayer.Character 
       ...
       and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if not HumanoidRootPart:FindFirstChild("MightHubTweenKaitun") then
            local Noclip = Instance.new("BodyVelocity")
            Noclip.Name = "MightHubTweenKaitun"
        ...
        if Char.Humanoid.Health <= Char.Humanoid.MaxHealth * 0.3 then
            pcall(function()
                tween:Cancel()
            end)
            ...
            until not Char:FindFirstChild("Humanoid") or Char.Humanoid.Health > Char.Humanoid.MaxHealth * 0.7
        ...
        -- On tween completion, clean up
        if tween then
            tween.Completed:Connect(function(status)
                if status == Enum.PlaybackState.Completed then
                    if HumanoidRootPart:FindFirstChild("MightHubTweenKaitun") then
                        HumanoidRootPart:FindFirstChild("MightHubTweenKaitun"):Destroy()
                    end
                end
            end)
        end
...
if not Char.HumanoidRootPart:FindFirstChild("MightHubTween") then
local Noclip = Instance.new("BodyVelocity")
...
if Char.HumanoidRootPart:FindFirstChild("MightHubTween") then
Char.HumanoidRootPart:FindFirstChild("MightHubTween"):Destroy()
end
end
end)
end)
