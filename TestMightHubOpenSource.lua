...
---- AntiBan
local function destroyScripts(scriptNames, parent)
    ...
    if game.Players.LocalPlayer.Character 
       ...
       and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        if game.Players.LocalPlayer.Character.Humanoid.Health <= game.Players.LocalPlayer.Character.Humanoid.MaxHealth * 0.3 then
            local OldY = HumanoidRootPart.CFrame.Y
            repeat
                task.wait()
        ...
        pcall(function()
            for _, v in pairs(game.Workspace.Enemies:GetChildren()) do
                ...
                if enemyHumanoid and enemyRoot and enemyHumanoid.Health > 0 then
                    if v.Name == MonFarm and (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - enemyRoot.Position).Magnitude <= 450 then
                        repeat
                            task.wait()
                            enemyRoot.CFrame = FarmPos
                            enemyRoot.CanCollide = false
                            enemyRoot.Size = Vector3.new(60, 60, 60)
                            enemyRoot.Transparency = 1
                            
                            -- Update enemy humanoid state
                            enemyHumanoid:ChangeState(11)
                            enemyHumanoid:ChangeState(14)
                            enemyHumanoid:ChangeState(16)
                            enemyHumanoid.JumpPower = 0
                            enemyHumanoid.WalkSpeed = 0

                            local animator = enemyHumanoid:FindFirstChild("Animator")
                            if animator then animator:Destroy() end
                        until (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - enemyRoot.Position).Magnitude > 400
                            or v.Name ~= MonFarm or not v.Parent or enemyHumanoid.Health <= 0
                    end
                end
            end
        end)
...
task.spawn(function()
    while task.wait(0.1) do  -- Thêm khoảng dừng nhỏ để giảm tải cho vòng lặp
        pcall(function()
            local target = nil  -- Biến này sẽ giữ đối tượng cần tấn công (quái vật hoặc người chơi)

            -- Kiểm tra kẻ thù trong Workspace.Enemies
            for _, v in pairs(game.Workspace.Enemies:GetChildren()) do
                local enemyHumanoid = v:FindFirstChild("Humanoid")
                local enemyRoot = v:FindFirstChild("HumanoidRootPart")

                if enemyHumanoid and enemyRoot and enemyHumanoid.Health > 0 then
                    local distance = (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - enemyRoot.Position).Magnitude
                    if distance <= 125 then
                        target = v  -- Chọn quái vật làm mục tiêu
                    end
                end
            end

            -- Kiểm tra người chơi trong Players nếu UsefastattackPlayers được bật
                ...
                for _, v in pairs(game:GetService("Players"):GetChildren()) do
                    if v ~= game.Players.LocalPlayer and v.Character then
                        ...
                        if enemyHumanoid and enemyRoot and enemyHumanoid.Health > 0 then
                            local distance = (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - enemyRoot.Position).Magnitude
                            if distance <= 125 then
                                -- Nếu chưa có quái vật, chọn người chơi làm mục tiêu
                                if not target then
                                    target = v
                                end
                            end
            ...
            if target then
                ...
                if targetHumanoid and targetRoot and targetHumanoid.Health > 0 then
                    NeedAttacking = true
                    repeat
                        task.wait(0.1)  -- Chờ một chút để tránh quá tải vòng lặp
                        AutoHaki()  -- Gọi AutoHaki nếu có
                        Click()     -- Gọi Click nếu có
                    until (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - targetRoot.Position).Magnitude > 125
                        or not target.Parent or targetHumanoid.Health <= 0
                    NeedAttacking = false
                end
            end
        end)
            ...
            pcall(function()
                            ...
                            if enemy:FindFirstChild("Humanoid") and enemy:FindFirstChild("HumanoidRootPart") and enemy.Humanoid.Health > 0 then
                                if enemy.Name == Ms then
                                    repeat
                                        ...
                                        MonFarm = enemy.Name
                                    until not _G.AutoKaitun or not enemy.Parent or enemy.Humanoid.Health <= 0
                                end
                            end
                        end
                    end
                ...
                for _, spawn in pairs(game:GetService("Workspace")["_WorldOrigin"].EnemySpawns:GetChildren()) do
                    if string.find(spawn.Name, NameMon) then
                        ...
                        Tween(CFrameMon) -- Teleporting to Mon spawn position local
                    end
                end
            end)
        end
