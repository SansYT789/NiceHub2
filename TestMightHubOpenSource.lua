...
function BringMob(MobCheck)
    pcall(function()
        for _, enemy in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
            if enemy:FindFirstChild("Humanoid") and enemy:FindFirstChild("HumanoidRootPart") and enemy.Humanoid.Health > 0 then
                if not string.find(enemy.Name, "Boss") and enemy.Name == MobCheck.Name and (enemy.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 450 then
                    repeat
                        task.wait(0.1)

                        -- Tùy chỉnh HumanoidRootPart của kẻ thù
                        enemy.HumanoidRootPart.Size = Vector3.new(60, 60, 60)
                        enemy.HumanoidRootPart.CanCollide = false
                        enemy.HumanoidRootPart.Transparency = 0
                        enemy.HumanoidRootPart.CFrame = MobCheck.HumanoidRootPart.CFrame

                        -- Đặt trạng thái và thuộc tính cho Humanoid của kẻ thù
                        enemy.Humanoid:ChangeState(11)  -- Giữ trạng thái cuối cùng
                        enemy.Humanoid.JumpPower = 0
                        enemy.Humanoid.WalkSpeed = 0

                        -- Kiểm tra và xóa Animator nếu tồn tại
                        local animator = enemy.Humanoid:FindFirstChild("Animator")
                        if animator then
                            animator:Destroy()
                        end

                    until not enemy.Parent or enemy.Humanoid.Health <= 0 or enemy.Name ~= MobCheck.Name
                end
            end
        end
    end)
