...
CombatFramework.activeController.timeToNextAttack = 0.5
...
if string.find(v.Name,MonFarm) and (v.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 400 then
if InMyNetWork(v.HumanoidRootPart) and InMyNetWork(v.HumanoidRootPart) then
...
loadstring(
    ...
    local oldNameCall = gt.__namecall
    setreadonly(gt, false)

    gt.__namecall = newcclosure(function(...)
        local args = {...}
        
        -- Check if the method being called is InvokeServer
        if getnamecallmethod() == "InvokeServer" then 
            -- Check if the second argument is "TAP"
            if tostring(args[2]) == "TAP" then
                -- Check if AimbotCheck and AimbotMobPosition are defined
                if AimbotCheck and AimbotMobPosition then
                    args[3] = AimbotMobPosition -- Modify the third argument
                end
            end
        end
        
        -- Call the original __namecall method with modified arguments
        return oldNameCall(unpack(args))
    end)

    setreadonly(gt, true) -- Restore metatable to read-only to prevent further modifications
]]
)()
...
until _G.AutoFarmAcceptQuest or not _G.AutoLevel or CheckIsRaiding() or not v.Parent or v.Humanoid.Health <= 0 or not game:GetService("Workspace").Enemies:FindFirstChild(v.Name)
end
end
end
Tween(CFrameMon)
