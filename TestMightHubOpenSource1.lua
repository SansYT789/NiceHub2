...
local Toggle = Tabs.MainFarm:AddToggle("MainFarmAcceptQuestToggle", {Title = "Accept Quest", Default = false })
spawn(function()
while task.wait() do
...
NeedAttacking = true
sethiddenproperty(game.Players.LocalPlayer, "SimulationRadius", math.huge)
until SelectFarmMode ~= "Level" or Options.MainFarmAcceptQuestToggle.Value or not Options.MainFarmStartFarmToggle.Value or not enemy.Parent or enemy.Humanoid.Health <= 0 or not questVisible
HackerNoclipFly = false
NeedAttacking = false
end
end
...
NeedAttacking = true
sethiddenproperty(game.Players.LocalPlayer, "SimulationRadius", math.huge)
until SelectFarmMode ~= "Bone" or Options.MainFarmAcceptQuestToggle.Value or not Options.MainFarmStartFarmToggle.Value or not enemy.Parent or enemy.Humanoid.Health <= 0 or not questVisible
HackerNoclipFly = false
NeedAttacking = false
end
end
...
CancelTween(Options.SubFarmKillRipIndraToggle.Value)
end
end
end)
...
local Toggle = Tabs.RaidEsp:AddToggle("RaidEspOneClickRaidToggle", {Title = "One Click Raid", Default = false })
spawn(function()
while wait() do
if Options.RaidEspOneClickRaidToggle.Value then
pcall(function()
...
until not getNextIsland() or not CheckIsRaiding() or not Options.RaidEspOneClickRaidToggle.Value
end
...
until not CheckIsRaiding() or not Options.RaidEspOneClickRaidToggle.Value or not v.Parent or v.Humanoid.Health <= 0
end
end
end
end)
else
CancelTween(Options.RaidEspOneClickRaidToggle.Value)
end
end
end)
