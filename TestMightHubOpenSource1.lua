...
local Tabs = {
    ...
    StackFarm = Window:AddTab({ Title = "Stack Farm", Icon = "settings" })
...
function Tween(P1)
    if game.Players.LocalPlayer.Character 
       ...
       and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
        ...
        -- Check and update HackerNoclipFly state
        if tween then
            HackerNoclipFly = tween.PlaybackState == Enum.PlaybackState.Playing
        else
...
local RandomCFrameChoices = {CFrame.new(0, PosY, 0),
...
CFrame.new(0, PosY, -20),
CFrame.new(20, PosY, 0),
CFrame.new(-20, PosY, 0)}

spawn(function()
    while wait(.15) do
        pcall(function()
            MethodFarm = RandomCFrameChoices[math.random(1, #RandomCFrameChoices)]
        end)
    end
end)
...
spawn(function()
    while game:GetService("RunService").Stepped:Wait() do
        ...
        if ac and ac.equipped and not CheckStun() then
            ...
            elseif UseFastAttackPlayersAndMob and Fast_Attack then
...
local SelectWeaponDropdown = Tabs.Settings:AddDropdown("WeaponChoseDropdown", {
Title = "Select Weapon",
Values = WeaponChose,
Multi = false,
Default = 1,
})
WeaponChoseDropdown:SetValue("FatBell Attack")
...
Options.MainFarmStartFarmToggle:SetValue(false)

spawn(function()
while task.wait() do
if Options.MainFarmStartFarmToggle.Value then
pcall(function()
-- Kiểm tra và teleport
local hasFruit = false
local hasElite = false

if Options.StackFarmFruitToggle.Value then
...
if Options.StackFarmEliteToggle.Value then
...
for _, elite in pairs(game:GetService("ReplicatedStorage"):GetChildren()) do
if elite:FindFirstChild("Humanoid") and elite:FindFirstChild("HumanoidRootPart") and elite.Humanoid.Health > 0 then
if elite.Name == "Diablo" or elite.Name == "Deandre" or elite.Name == "Urban" then
hasElite = true
else
...
until player.Character:FindFirstChild(fruit.Name) or player.Backpack:FindFirstChild(fruit.Name) or not fruit or not hasFruit
end
end
end
if hasElite then
...
until not hasElite or elite.Humanoid.Health <= 0 or not elite.Parent
HackerNoclipFly = false
NeedAttacking = false
end
end
...
-- Nếu không có trái cây, kiểm tra nhiệm vụ
if not hasFruit and not hasElite then
CheckLevel()
...
---- [ Stack Farm Tabs ] ----
local Toggle = Tabs.StackFarm:AddToggle("StackFarmFruitToggle", {Title = "Auto Fruit", Default = false })
Options.StackFarmFruitToggle:SetValue(false)

local Toggle = Tabs.StackFarm:AddToggle("StackFarmEliteToggle", {Title = "Auto Elite", Default = false })
Options.StackFarmEliteToggle:SetValue(false)
...
local TpSpeedBikiniDropdown = Tabs.Settings:AddDropdown("TpSpeedDropdown", {
Title = "Tween Speed",
Values = TpSpeed,
Multi = false,
Default = 5,
})
TpSpeedDropdown:SetValue("300")
...
local FatBellDelayModeDropdown = Tabs.Settings:AddDropdown("FatBellModeDropdown", {
Title = "Fast Attack Mode",
Values = FatBellDelayMode,
Multi = false,
Default = 4,
})
FatBellModeDropdown:SetValue("FatBell Attack")
...
Options.SettingsFastAttackToggle:SetValue(true)
FatBellToggle:OnChanged(function()
Fast_Attack = Options.SettingsFastAttackToggle.Value
end)

local BringNgonLanhToggle = Tabs.Settings:AddToggle("SettingsBringMobToggle", {Title = "Bring Mob", Default = true })
Options.SettingsBringMobToggle:SetValue(true)
spawn(function()
while task.wait(.125) do
if Options.SettingsBringMobToggle.Value then
pcall(function()
for _, enemy in pairs(game:GetService("Workspace").Enemies:GetChildren()) do
if not string.find(enemy.Name,"Boss") and enemy.Name == MonName and (enemy.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <= 450 then
...
enemy.Humanoid:ChangeState(14) -- Physics
...
local AutoClickToggle = Tabs.Settings:AddToggle("SettingsAutoClickToggle", {Title = "Auto Click", Default = false })
...
InterfaceManager:SetFolder("MightHubConfig")
SaveManager:SetFolder("MightHubConfig/BloxFruits")
