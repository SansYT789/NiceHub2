...
print("Đang kiểm tra Character:", player and player.Character)
...
function getPortal(check)
    ...
    local TableLocations = {
        [7449423635] = {
            ["Castle On The Sea"] = Vector3.new(-5058.775, 314.516, -3155.883),
    ...
    local locations = TableLocations[game.PlaceId]
    if not locations then return nil end -- Không có dữ liệu cho map hiện tại

    -- Tìm cổng gần nhất
    for _, position in pairs(locations) do
        local distance = (position - vcspos).Magnitude
        if distance < minDistance then
            minDistance, closestPortal = distance, position
        end
    end

    -- Kiểm tra khoảng cách từ người chơi đến cổng
...
pcall(function()
game.ReplicatedStorage.Remotes.CommF_:InvokeServer("requestEntrance", check1)
game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame += Vector3.new(0, 50, 0)
task.wait(0.5)
end)
end
...
pcall(function()
if not P1 or not game.Players.LocalPlayer.Character:FindFirstChild("Humanoid") or not game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then return end
...
game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = P1
until (game.Players.LocalPlayer.Character.HumanoidRootPart.Position - P1.Position).Magnitude < 2500 and game.Players.LocalPlayer.Character.Humanoid.Health > 0
end
end)
end

function hasBlacklistItem()
for i,v in pairs(BlacklistItemTp) do
for x,y in pairs(NameFruitList) do
if game:GetService("Players").LocalPlayer.Backpack:FindFirstChild(v) or game.Players.LocalPlayer.Character:FindFirstChild(v) or game:GetService("Players").LocalPlayer.Backpack:FindFirstChild(y) or game.Players.LocalPlayer.Character:FindFirstChild(y) then
return true
else
return false
end
...
function TweenToTarget(P1, Speed)
pcall(function()
...
game.Players.LocalPlayer.Character.HumanoidRootPart, TweenInfo.new((P1.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude / Speed, Enum.EasingStyle.Linear), 
{CFrame = P1})
tween:Play()
...
function CheckAndTeleport(P1, Speed)
    if (P1.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude > 2500 and BYPASSTELEPORT and not CheckIsRaiding() and not hasBlacklistItem() then
        BypassTp(P1) -- Teleport khi quá xa
    else
        TweenToTarget(P1, Speed)
    end
end

function Tween(P1)
pcall(function()
if not (game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChild("Humanoid") and game.Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart") and game.Players.LocalPlayer.Character.Humanoid.Health > 0) or not P1 then return end
...
CheckAndTeleport(P1, TweenSpeed)
end)
end

function CancelTween(target)
    if not target then
        if game.Players.LocalPlayer.Character:FindFirstChild("Head") and game.Players.LocalPlayer.Character:FindFirstChild("Head"):FindFirstChild("BodyVelocity") then
            game.Players.LocalPlayer.Character:FindFirstChild("Head").BodyVelocity:Destroy()
        end
...
MainFarmSelectWeaponDropdown:SetValue("...")
...
HackerNoclipFly = true
until not Options.SubFarmOpenHakiPadToggle.Value or not part
HackerNoclipFly = false
end
end)
else
...
local IslandsTpWorldSeaDropdown = Tabs.Islands:AddDropdown("IslandsTpWorldSeaDropdown", {
Title = "Teleport World",
Values = Seas,
Multi = false,
Default = 1
})
IslandsTpWorldSeaDropdown:SetValue("...")
IslandsTpWorldSeaDropdown:OnChanged(function(Value)
    -- Đảm bảo `Value` không rỗng và đúng định dạng
    local seaNumber = tonumber(tostring(Value):match("%d+")) -- Lấy số từ chuỗi "Sea X"
    ...
    if seaNumber then
        ...
        warn("Invalid sea selected: " .. tostring(Value))
    end
end)
